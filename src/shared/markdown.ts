 ...
 *   `<strong>x</strong>`.
 * - Fenced blocks (`` ``` ``), inline code (`` ` ``) and images are extracted
 *   to placeholders before escaping so their content is never re-interpreted.
 ...
 *
 * Supported syntax (covers >99% of model output, plus GFM):
 * `**bold**`, `__bold__`, `*italic*`, `_italic_`, `` `code` ``, fenced code,
 * `[text](url)`, `#`–`######` headings, `> blockquote`, `*`/`-`/`+`
 * unordered lists, `1.` ordered lists, `---` hr, paragraphs and soft line
 * breaks; plus tables, task lists, nested lists and images:
 ...
 *
 * Public API: `renderMarkdown`, `renderMarkdownToElement` and the
 * `markdownToHtml` alias. Unknown syntax falls through as plain text.
 */
